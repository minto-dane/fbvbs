...
In the current repository, `hypervisor/src/vmx.c` has been reduced to capability probing and leaf-exit observation, while `hypervisor/src/vm_policy.c` remains only as a temporary C mirror for synthetic VM exit response shaping. Ada/SPARK now owns the authoritative policy model via `FBVBS.VM_Policy` and `FBVBS.VM_Exit_Encoding`, and the Ada executable checks concrete exit payload semantics for interrupt, PIO, MMIO, CR, MSR, EPT, shutdown, halt, and fault cases. The Ada side now also has an explicit selected-call hypercall dispatcher (`FBVBS.Hypercall_Dispatcher`), a memory/W^X policy model (`FBVBS.Memory`), a KSI shadow-update model (`FBVBS.KSI_Shadow`), and selected diagnostic query modeling (`FBVBS.Diagnostics`) so more orchestration remains on the Ada side. The target end state remains that any surviving C is reduced to leaf wrappers with a fixed ABI.
