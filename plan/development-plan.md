...
├─────────────────────────────────────────────────────────────┤
│  Layer 1: マイクロハイパーバイザー（C11 + ACSL retained C）   │
│    ├── パーティション管理                                     │
│    ├── 第二レベルページング                                   │
│    ├── IOMMU ドメイン制御                                    │
│    ├── CPU 制御強制                                          │
...
### Phase 0: マイクロハイパーバイザー基盤（進行中）

**目標:** FreeBSD を deprivileged host として起動し、最低1つの信頼サービスパーティションを稼働させる。

**成果物:**
- C11 + ACSL retained C マイクロハイパーバイザー: パーティション管理、第二レベルページング、IPC、VMCALL ハンドリング、一次監査ログ（UART）
...
**検証:**
- `make -C hypervisor analyze` による GCC `-fanalyzer`
- `make -C hypervisor test` による retained C unit test
- `make -C hypervisor proof` による Frama-C WP 実行経路の維持

**未完了ブロッカー:**
- `KCI_SET_WX` の artifact bytes と execute 対象 GPA の byte-backed binding
- authoritative な IOMMU / interrupt remapping / boot integrity bring-up
- device qualification matrix, ACS 検証, FLR/reset, MSI/MSI-X 制御を伴う passthrough
- Frama-C WP plugin を含む再現可能な proof 環境

**状態:** 進行中。retained C 実装は未完成機能を fail-closed にしているが、本番宣言条件は未達。
...
1. Phase 0 blocker の解消
2. `KCI_SET_WX` の page binding と execute 付与境界の完成
3. IOMMU / interrupt remapping / boot integrity の実機 bring-up
4. device qualification と safe passthrough の実装
