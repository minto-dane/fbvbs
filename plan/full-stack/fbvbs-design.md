...
本書は、FreeBSD カーネルが侵害された後でも、いくつかの中核的なセキュリティ不変条件を維持することを目的とした仮想化ベースの保護基盤 FBVBS の仕様である。ここでいう「完全仕様」とは、単なる構想説明ではなく、実装者、監査者、運用者、ならびに自動実装エージェントが、同一の設計判断、同一の責務分担、同一の保証境界、同一の検証要求に基づいて作業できる水準の文書を意味する。本書は、そのために必要な要求定義、脅威モデル、アーキテクチャ、コンポーネント境界、更新機構、監査機構、仮想化統合、暗号方針、品質基準、および残余リスクを、相互に矛盾しない形で定義する。

FBVBS はFreeBSD を意図的に非信頼ホストとして扱い、最上位特権を持つ最小のマイクロハイパーバイザーが、コード整合性、カーネル重要状態、鍵素材、DMA 境界、および監査証跡を、FreeBSD の権限から切り離して保持する。その意味で FBVBS は、FreeBSD 自体を守る設計ではなく、FreeBSD 侵害後にも残るべき保護性質を、より狭い信頼境界の内側で維持する設計である。

本設計は以下の四つの原則に基づく。第一に、監査ログは FreeBSD ホスト侵害から独立した観測経路を持つ一次監査ログと、FreeBSD から可視であるが改ざん耐性を持たないミラーログとを明確に分離する。UART のみに依存する設計では、ホスト侵害時の監査証跡が保証されない。第二に、翻訳整合性について、Intel 実装では HLAT を必須とし、AMD 実装については NPT、ページテーブル更新トラップ、シャドウ翻訳経路、ならびに必要に応じて SEV-SNP の機構を組み合わせて、同等のセキュリティ目標を達成する構成を要求する。この同等性は実装と検証によって立証されなければならない。第三に、更新機構は特定の搬送手段に固定せず、署名付き成果物と署名付きマニフェストの検証に依存する、搬送手段非依存のモデルを採る。第四に、暗号ライブラリについては、検証志向の限定的 primitive 実装を優先評価しつつ、それだけで TLS、SSH、IPsec のような全プロトコル群を完結させる前提を採らない。
...
運用構成の分類は `plan/shared/fbvbs-deployment-profiles.md` を正とする。そこでは
次の 2 プロファイルを定義する。

1. FreeBSD 保護を目的とする FBVBS スタック型マイクロハイパーバイザー
2. 共通 retained-C 保護基盤を再利用するスタンドアロン FBVBS マイクロハイパーバイザー

本書の要求適合主張は、原則として前者、すなわち FreeBSD を非信頼ホストとして
減権し、その上で KCI、KSI、IKS、SKS、UVS 等のサービス平面によって FreeBSD
侵害後の中核不変条件を維持する FBVBS スタックを対象とする。後者は姉妹プロダクト
であり、共有するのは保護基盤、パーティション原語、監査、検証規律であって、
FreeBSD 保護意味論やサービス平面要件そのものではない。スタンドアロン構成の
製品要求、品質方針、OCS/VCD 管理面要件は
...
`plan/standalone/operations/operator-control-plane.md` を正とする。したがって、スタンドアロン
構成は、本書のフルスタック要件を別途満たさない限り、FBVBS スタック適合を
主張してはならない。
...
ABI v1 のフルスタック適合 service identity は `SERVICE_KIND_KCI`、`SERVICE_KIND_KSI`、`SERVICE_KIND_IKS`、`SERVICE_KIND_SKS`、`SERVICE_KIND_UVS` の五つである。trusted-service partition の `service_kind` は `PARTITION_MEASURE` 成功時に当該 manifest の `service_kind` から確定し、以後不変とする。測定前の trusted-service partition は `SERVICE_KIND_NONE` として診断表示される。共通 ABI には、スタンドアロン FBVBS マイクロハイパーバイザー向けの管理面拡張として `SERVICE_KIND_OCS` を含めてよいが、これは本書のフルスタック適合 service には数えず、KCI/KSI/IKS/SKS/UVS の代替を主張してはならない。`SERVICE_KIND_OCS` を実装する場合、その意味論、権限、診断可視性はスタンドアロン文書群を正とする。Audit Relay Service は ABI v1 の service kind には含めず、実装する場合は内部拡張とし、外部 ABI 適合主張から除外する。`DIAG_GET_PARTITION_LIST` ではそのような内部拡張 partition を列挙してはならない。
...
ただし、trusted service 群を載せる前提となる基盤マイクロハイパーバイザー自身が未完成であってはならない。少なくとも、authoritative な second-stage translation 管理、IOMMU と interrupt remapping の有効化、partition/vCPU 破棄時のゼロ化、owner-bound command page 検証、race-free な command state machine、per-CPU の VM entry/exit 緩和、一次監査ログの OOB 観測経路は、サービス実装以前に成立していなければならない。これらのいずれかを欠く実装は、service ABI が存在していても production-grade の FBVBS 基盤とはみなしてはならない。
...
| `Measured` | `PARTITION_LOAD_IMAGE` | `Loaded` | authoritative image object の配置成功 | guest VM または trusted-service の partition-loadable executable artifact に対してコードと初期データを配置する。ABI v1 retained-C release profile では Appendix L.1.E の fixed ELF64 `ET_EXEC` loader を用い、authoritative image object が得られない場合は fail-closed しなければならない |
...
| `Running`/`Runnable`/`Loaded`/`Quiesced` | 致命障害検出 | `Faulted` | なし | 一次監査ログ必須。`Created`/`Measured` からの直接 fault は禁止（ライフサイクルスキップ防止） |
| `Faulted` | `PARTITION_RECOVER` | `Runnable` | 再測定、ゼロ化、必要状態復元成功 | 回復失敗時は `Faulted` 維持 |
| `Created`/`Measured`/`Loaded`/`Runnable`/`Quiesced`/`Faulted` | `PARTITION_DESTROY` | `Destroyed` | 資源回収とゼロ化成功 | `Running` からの直接破棄要求は fail-closed とし `INVALID_STATE`。caller は先に `PARTITION_QUIESCE` を成功させなければならない |
...
初回 `Running` への遷移時の CPU 初期状態は凍結する。通常の loadable partition entry では、`RIP` は `PARTITION_LOAD_IMAGE` の `entry_ip` で決定される。ABI v1 では `entry_ip=0` の場合、manifest 内 `entry_ip` を用いる。`entry_ip!=0` の場合、その値は manifest 内 `entry_ip` と一致しなければならず、不一致は `MEASUREMENT_FAILED` とする。`RSP` は同 call で指定した `initial_sp` とする。`RFLAGS` は `0x0000000000000002` とし、bit 1 を 0 にしてはならない。`CR0` と `CR4` はそれぞれ基底値 `0x80010033`、`0x000006f0` に、その時点の hypervisor pin policy（must-be-1 / must-be-0 の両方）を適用した値を初期値とする。したがって caller が観測可能な初期値はプラットフォーム security profile により狭められてよく、少なくとも pinned bit に反する値で開始してはならない。汎用レジスタは `RIP` と `RSP` を除き 0、`XMM/YMM/ZMM` 状態は 0、`FS.base`/`GS.base` は 0、セグメントは flat 64-bit model、`CR3` は当該パーティションの第二レベル変換と整合する hypervisor 管理値とする。`RSI` のみ例外として bootstrap page GPA を保持してよい。caller はこれ以外の初期 CPU 状態に依存してはならない。`PARTITION_KIND_FREEBSD_HOST` は boot seed 後に host callsite table から初期 `RIP` を構成し、この `PARTITION_LOAD_IMAGE` 経路を通らない。build が authoritative bytes を供給できない、または Appendix L.1.E の固定 loader 規約を満たせない場合は、`PARTITION_LOAD_IMAGE` を成功させてはならない。
...
メモリ所有権も同様に厳密でなければならない。ある物理ページは、同時に二つの信頼サービスの可変領域であってはならない。共有が必要な場合は、明示的な共有ページとして登録し、そのページごとに片方向または両方向のアクセス権を定義する。FreeBSD との共有は原則として読み取り専用ミラーか、一時的な入出力バッファに限定される。鍵サービスの内部ページ、署名検証用一時バッファ、更新マニフェスト検証ページなどを、FreeBSD が直接マップすることは禁止される。`MEMORY_ALLOCATE_OBJECT` で生成された `memory_object_id` は、生成した caller partition に所有権が固定され、ABI v1 では ownership transfer を許可しない。したがって `MEMORY_MAP`、`MEMORY_SET_PERMISSION`、`MEMORY_REGISTER_SHARED`、`MEMORY_RELEASE_OBJECT`、`VM_MAP_MEMORY` を要求できるのは当該 owner caller のみとし、peer partition が registration だけを根拠に直接 object を map/unregister/release してはならない。

shared registration は capability metadata であり、memory reservation ではない。したがって `MEMORY_REGISTER_SHARED` 成功時点では `memory_limit_bytes` も `mapped_bytes` も消費してはならず、実際に `MEMORY_MAP`、`VM_MAP_MEMORY`、または command separate-output のために map された時点でのみ page-aligned bytes を accounting しなければならない。shared registration の解除または owner partition の destroy は、まず実際の peer mapping を剥がし、その後に registration metadata を削除することで accounting 整合性を保たなければならない。`MEMORY_UNREGISTER_SHARED` は owner の自己 map を理由に拒否してはならないが、targeted registration なら対応 peer partition の live mapping、broadcast registration なら owner 以外の live mapping または reserved separate-output 参照（`MEMORY_REGISTER_SHARED` により別出力ページとして予約された live output-page binding） が残る間は成功してはならない。
...
以下の subsection は、本設計が利用しなければならない x86_64 CPU セキュリティ機能の完全な要求を定義する。これらの機能は `plan/full-stack/cpu-sec.md` で網羅的に調査された 81 以上の機能に基づく。マルチコア環境において、すべての緩和策はコアごとに適用され、コア間のレース条件を許容してはならない。

### 21.1. 投機実行緩和戦略

投機実行脆弱性は、ハイパーバイザーにとって最も複雑かつ継続的に進化する脅威領域である。本設計は、すべての特権遷移境界（ユーザー→カーネル、ゲスト→ホスト、ゲスト→ゲスト）において緩和策を適用しなければならない。

**Spectre v2（Branch Target Injection）緩和:**

- Intel: eIBRS（Enhanced IBRS）を必須とする。`IA32_ARCH_CAPABILITIES` MSR (0x10A) bit 1 (`IBRS_ALL`) で検出する。起動時に `IA32_SPEC_CTRL[0]=1` を設定し、以後変更しない。eIBRS は特権境界を跨ぐ間接分岐予測を自動制限するが、Branch History Buffer (BHB) を分離しないため、BHI 緩和を別途必要とする。
- AMD: AutoIBRS を必須とする（Zen 4 以降）。`CPUID Fn8000_0021:EAX[bit 8]` で検出し、`EFER.AUTOIBRS` (bit 21) で有効化する。AutoIBRS は VMEXIT 時に内部 Return Address Stack を自動クリアする（Intel eIBRS にはない利点）。Zen 1–3 では legacy IBRS (`IA32_SPEC_CTRL[0]=1`) を遷移ごとに切り替える方式とするが、性能コストが大きいため Zen 4 以降を推奨構成とする。

**IBPB（Indirect Branch Predictor Barrier）:**

- `IA32_PRED_CMD` (MSR 0x49) bit 0 への書込みにより、間接分岐予測器のすべてのエントリをフラッシュする。eIBRS/AutoIBRS であっても、同一特権レベルのコンテキスト間（ゲスト→ゲスト）切替時には IBPB が必須である。
- AMD Zen 4 以降では IBPB が RSB もクリアする（`IBPB_RET`: `CPUID Fn8000_0008:EBX[30]`）。旧 AMD では RSB フィルが別途必要。
- マルチコア環境では、各物理コアがパーティション切替時に独立して IBPB を発行しなければならない。

**BHI（Branch History Injection）緩和:**

- Intel: `BHI_DIS_S` (`IA32_SPEC_CTRL[10]`) を有効化する。`CPUID.7.2.EDX[4]` (`BHI_CTRL`) で検出する。BHI_DIS_S 非対応の旧 CPU では、syscall エントリおよび VMEXIT 時にソフトウェア BHB クリアシーケンス（約 200 命令）を実行しなければならない。
- AMD: BHI の影響を受けない。

**PBRSB（Post-Barrier RSB Predictions）緩和:**

- Intel eIBRS 搭載 CPU（Skylake 派生～Alder Lake）において、VMEXIT 後にゲスト制御の CALL ターゲットが最初の post-VMEXIT リターン予測に影響しうる。`IA32_ARCH_CAPABILITIES[24]` (`PBRSB_NO`) が立っていない場合、VMEXIT 直後に軽量 CALL シーケンスを実行しなければならない。
- AMD: AutoIBRS が VMEXIT 時に Return Address Stack を自動クリアするため、影響なし。

**STIBP（Single Thread Indirect Branch Predictors）:**

- SMT（Simultaneous Multi-Threading）有効時に、兄弟論理スレッド間の分岐予測クロス影響を防止する。Intel eIBRS はカーネルモードで暗黙的に STIBP を提供するが、AMD AutoIBRS は提供しない。
- AMD 構成では `IA32_SPEC_CTRL[1]=1` を明示設定しなければならない。

**SSBD（Speculative Store Bypass Disable / Spectre v4）:**

- `IA32_SPEC_CTRL[2]` で有効化。投機的ストア→ロード転送バイパスを防止する。性能コストが大きいため、JIT サンドボックスや非信頼コード実行環境でのみ有効化を推奨する。AMD では `VIRT_SPEC_CTRL` (MSR 0xC001_011F) も利用可能。

**RRSBA_DIS_S（Restricted Return Stack Buffer Alternate）:**

- RSB アンダーフロー時の BTB フォールバック予測を防止する。`IA32_SPEC_CTRL` の `RRSBA_DIS_S`/`RRSBA_DIS_U` ビットで制御。RRSBA 挙動を持つプロセッサで retpoline を使用する場合に必要。

**RSB フィル:**

- VMEXIT ごとに RSB を 32 エントリで充填しなければならない（Intel）。AMD AutoIBRS は自動クリアするが、Zen 1–3 では明示フィルが必要。RSB フィルは各物理コアで独立に実行する。

**L1D フラッシュ（L1TF / Foreshadow 緩和）:**

- `IA32_FLUSH_CMD` (MSR 0x10B) bit 0 で L1 データキャッシュをフラッシュする。L1TF 脆弱な CPU（`IA32_ARCH_CAPABILITIES[0]` = `RDCL_NO` が立っていない Whiskey Lake 以前）では VM entry 時に必須。

**VERW / MD_CLEAR（MDS、TAA、MMIO Stale Data 緩和）:**

- MDS 脆弱な CPU（`CPUID.7.0:EDX[10]` で `MD_CLEAR` サポート確認）では、すべての特権遷移時に VERW 命令を実行しなければならない。TAA 脆弱な CPU では TSX 無効化（`IA32_TSX_CTRL` MSR 0x122）または VERW が必要。MMIO Stale Data 脆弱な CPU では `FB_CLEAR` サポートを `IA32_ARCH_CAPABILITIES[17]` で確認。

**Retbleed 緩和:**

- AMD Zen 1–3 および Intel Skylake 系で影響。untrained return thunk または IBRS で緩和。

**SRSO / Inception 緩和:**

- AMD Zen 1–4 で影響（Zen 5 は非影響）。safe-ret thunk と IBPB で緩和。`BP_CFG` MSR (0xC001102E) で検出。

**Downfall / GDS 緩和:**

- Intel Skylake–Tiger Lake で影響。マイクロコード更新で gather 命令の一時的遷移をブロック。`IA32_ARCH_CAPABILITIES[26]` (`GDS_NO`) で免疫確認。

**Zenbleed 緩和:**

- AMD Zen 2 のみ影響。マイクロコードまたは `DE_CFG` (MSR 0xC0011029) bit 9 で緩和。

**LFENCE シリアライズ化（AMD）:**

- AMD では `DE_CFG` (MSR 0xC001_1029) bit 1 を起動時に設定し、LFENCE をディスパッチシリアライズ化しなければならない（Intel では常にシリアライズ）。Spectre v1 緩和の LFENCE バリアに必要。

**IA32_ARCH_CAPABILITIES の完全解析:**

- マイクロハイパーバイザーは起動時に `IA32_ARCH_CAPABILITIES` MSR (0x10A) の 30 以上のビットを完全にパースし、各 CPU の脆弱性免疫状態を判定しなければならない。この情報は per-CPU 構造体に記録し、各緩和策の有効化/無効化を決定する。マルチコア環境では、すべての論理プロセッサで同一の解析を実行し、結果の一貫性を検証しなければならない（マイクロコードバージョン差によるコア間不一致の検出）。

**継続的更新への対応:**

- 投機実行脆弱性は四半期ごとに新種が公開される（2025 年だけで BPI、TSA、VMScape が公開された）。本設計は、脆弱性検出テーブルを `IA32_ARCH_CAPABILITIES` ビットと AMD `CPUID Fn8000_0008:EBX` / `Fn8000_0021:EAX` ビットにキーしたバージョン管理対象構造として維持し、マイクロコード更新に追従する仕組みを持たなければならない。

### 21.2. 制御フロー完全性: CET Shadow Stack および IBT

**Intel CET Shadow Stack (CET-SS):**

- `CPUID.(EAX=7,ECX=0):ECX[bit 7]` で検出。`CR4.CET` (bit 23) で有効化。
- ハードウェア管理の二次スタックにより、CALL 時にリターンアドレスを通常スタックとシャドウスタックの両方に push し、RET 時に比較する。不一致時に `#CP`（制御保護例外）を発生させる。
- 各特権レベル (PL0–PL3) に専用の Shadow Stack Pointer (`IA32_PL0_SSP` – `IA32_PL3_SSP`, MSR 0x6A4–0x6A7) を持つ。
- マイクロハイパーバイザーは自身のシャドウスタックを EPT で保護し、ゲストのシャドウスタックページを EPT のスーパーバイザーシャドウスタック属性でマークしなければならない。
- パーティション遷移時にはシャドウスタック MSR を per-vCPU 状態として保存・復元しなければならない。マルチコア環境では、各物理コアが独立したシャドウスタック状態を管理する。
- `IA32_S_CET` (MSR 0x6A2) と `IA32_U_CET` (MSR 0x6A0) はスーパーバイザーモードおよびユーザーモードの CET 構成を制御する。
- `IA32_INTERRUPT_SSP_TABLE_ADDR` (MSR 0x6A8) は割込みシャドウスタックテーブルを指す。
- AMD は同一 ISA のシャドウスタックをサポート。`SSSCheck` ビット (`CPUID Fn8000_000A_EDX[bit 19]`) により Nested Page Table 経由のスーパーバイザーシャドウスタック検証が可能。
- FreeBSD カーネルに対して `CR4.CET` をピン留めし、ゲストが CET を無効化することを禁止しなければならない。

**Intel CET-IBT（Indirect Branch Tracking）:**

- `CPUID.(EAX=7,ECX=0):EDX[bit 20]` で検出。Intel 専用機能。
- 間接 CALL/JMP の直後に `ENDBR64` 命令が存在しない場合に `#CP` を発生させる。JOP/COP 攻撃を防止する。
- マイクロハイパーバイザー自身のコードで IBT を有効化し、ゲストカーネルにも適用を推奨する。
- AMD では IBT が存在しないため、前方エッジ CFI はソフトウェア機構（FineIBT 等）に依存する。

### 21.3. VM Exit および VM Entry 緩和シーケンス

マルチコア環境において、各物理コアは VM exit/entry 時に以下のシーケンスを厳密に実行しなければならない。このシーケンスはコア間で独立であり、ロックを必要としない。

**VM Exit 緩和シーケンス（ゲスト→ハイパーバイザー）:**

1. IBPB 発行（ゲスト→ホスト予測バリア）。異なるパーティション間の切替時に必須。同一パーティション内の短時間 exit では条件付き省略可。
2. RSB フィル（32 エントリ、Intel）。AMD AutoIBRS は自動クリア。Zen 1–3 では明示フィル。
3. PBRSB シーケンス実行（影響のある Intel CPU のみ。`IA32_ARCH_CAPABILITIES[24]` = 0 の場合）。
4. VERW 実行（MD_CLEAR 必要 CPU のみ。`CPUID.7.0:EDX[10]` で判定）。
5. BHB クリアシーケンス実行（BHI_DIS_S 非対応の Intel CPU のみ）。
6. ゲスト `IA32_SPEC_CTRL` 状態を per-vCPU 構造体に保存。
7. ホスト `IA32_SPEC_CTRL` 状態を復元。
8. デバッグレジスタ (DR0–DR7) をゲスト状態から保存し、ホスト状態を復元。
9. CET MSR（`IA32_S_CET`, `IA32_U_CET`, `IA32_PLx_SSP`）をゲスト状態から保存し、ホスト状態を復元。
10. 性能カウンタ状態を隔離（LBR、Intel PT、AMD IBS の MSR を退避）。

**VM Entry 緩和シーケンス（ハイパーバイザー→ゲスト）:**

1. L1D フラッシュ（L1TF 脆弱 CPU のみ。`IA32_ARCH_CAPABILITIES[0]` = 0 の場合）。
2. ゲスト `IA32_SPEC_CTRL` 状態を復元。
3. ゲスト CET MSR を復元。
4. ゲストデバッグレジスタを復元。
5. ゲスト性能カウンタ状態を復元（許可されている場合）。

このシーケンスの各ステップは、CPU ごとの脆弱性プロファイルに基づいて条件付きで実行される。脆弱性プロファイルは起動時に構築された per-CPU 構造体から参照する。

### 21.4. MSR ビットマップと安全重要 MSR のインターセプト

Intel VMCS MSR ビットマップおよび AMD VMCB MSR Permission Map (8KB) を用いて、セキュリティ上重要な MSR へのゲストアクセスを個別制御する。以下の MSR は無条件でインターセプト対象としなければならない:

| MSR | 名称 | インターセプト理由 |
|-----|------|--------------------|
| 0xC0000080 | `IA32_EFER` | NXE、LME、SCE ビットの保護 |
| 0xC0000081 | `IA32_STAR` | syscall エントリポイントの改竄防止 |
| 0xC0000082 | `IA32_LSTAR` | 64-bit syscall エントリの改竄防止 |
| 0xC0000083 | `IA32_CSTAR` | 互換モード syscall エントリの改竄防止 |
| 0xC0000084 | `IA32_FMASK` | syscall フラグマスクの改竄防止 |
| 0x48 | `IA32_SPEC_CTRL` | 投機実行制御の不正変更防止 |
| 0x49 | `IA32_PRED_CMD` | IBPB の不正発行検出 |
| 0x10A | `IA32_ARCH_CAPABILITIES` | 脆弱性免疫偽装の防止 |
| 0x6A0–0x6A8 | CET 関連 MSR | シャドウスタック構成の保護 |
| 0x122 | `IA32_TSX_CTRL` | TSX 有効化/無効化の制御 |
| 0x10B | `IA32_FLUSH_CMD` | L1D フラッシュ制御 |
| 0x1A0 | `IA32_MISC_ENABLE` | 機能有効化ビットの保護 |
| 0x174–0x176 | `IA32_SYSENTER_*` | sysenter エントリの改竄防止 |
| 0x277 | `IA32_PAT` | メモリ型の不正変更防止 |

さらに、AMD 固有の MSR として `VIRT_SPEC_CTRL` (0xC001_011F)、`DE_CFG` (0xC0011029)、`BP_CFG` (0xC001102E) もインターセプト対象に含めなければならない。

### 21.5. VPID/ASID による TLB 分離

- Intel VPID（Virtual Processor Identifier）: VMCS に 16 ビットの VPID を設定し、vCPU ごとに TLB エントリをタグ付けする。異なるパーティション間での TLB 情報漏洩を防止する。VPID=0 はマイクロハイパーバイザー自身に予約する。
- AMD ASID（Address Space Identifier）: VMCB に ASID を設定し、同等の TLB 分離を提供する。
- マルチコア環境では、各物理コア上で動作する vCPU に一意の VPID/ASID を割り当てなければならない。パーティション切替時に異なるパーティションの vCPU が同一の VPID/ASID を共有することがないよう管理する。
- VPID/ASID を用いない構成では、パーティション切替ごとに完全な TLB フラッシュが必要となり、性能コストが許容不能となる。

### 21.6. デバッグレジスタおよび性能カウンタの分離

**デバッグレジスタ (DR0–DR7):**

- ゲストがホストアドレスにブレークポイントを設定することを防止するため、MOV-DR をインターセプトしなければならない。
- デバッグレジスタ状態は per-vCPU 構造体に保存・復元し、ゲスト状態がホスト状態に漏洩してはならない。
- マルチコア環境では、各コアがデバッグレジスタの独立した save/restore を行う。

**性能カウンタ (RDPMC):**

- LLC/キャッシュサイドチャネルによる cross-VM 情報漏洩を防止するため、`CR4.PCE` を 0 にピン留めし、RDPMC をインターセプトしなければならない。

**Intel PT（Processor Trace）:**

- 完全な制御フロートレースを提供し、情報漏洩リスクがある。PT 構成 MSR をインターセプトし、ゲストが無制限に PT を使用することを禁止しなければならない。ただし、ハイパーバイザーが CFI 監視目的で PT を使用することは許可される。

**AMD IBS（Instruction-Based Sampling）:**

- サンプリングベースの情報漏洩リスクがある。IBS MSR をインターセプトし、特権アクセスに制限しなければならない。

**LBR（Last Branch Records）:**

- 分岐トレースによる制御フロー漏洩を防止する。AMD は VMRUN/VMEXIT 時に LBR MSR を自動保存/復元する（LBR Virtualization）。Intel Architectural LBR (Alder Lake 以降) は標準化フォーマットを提供。両ベンダとも per-vCPU 保存/復元が必要。

**MTF（Monitor Trap Flag, Intel）:**

- シングルステップ実行に使用可能だが、TDX-Step 攻撃ベクタとなりうる。EPT フック命令リプレイにのみ使用し、外部公開しない。

### 21.7. メモリ保護機能の強制

**NX ビット（AMD NX / Intel XD）:**

- PTE bit 63。W^X の基盤。`EFER.NXE` をピン留めし、ゲストが無効化することを禁止。

**SMEP / SMAP ピニング:**

- `CR4.SMEP` (bit 20) と `CR4.SMAP` (bit 21) を VMCS/VMCB CR4 ゲスト/ホストマスクでピン留めする。ゲストがこれらのビットをクリアする試みはトラップし、拒否しなければならない。

**UMIP（User-Mode Instruction Prevention）:**

- `CR4.UMIP` (bit 11) を有効化し、ユーザーモードからの SGDT/SIDT/SLDT/SMSW/STR をブロックする。KASLR 情報漏洩を防止する。

**PCID（Process Context Identifiers）:**

- CR3 の 12 ビットタグにより TLB エントリを分離する。KPTI 性能に重要。ゲストの PCID 使用を許可するが、ハイパーバイザーは PCID の割当を監視する。

**PKU / PKS（Protection Keys）:**

- PKU（ユーザーページ用保護キー）: PTE bits 62:59 の 4 ビットキー。WRPKRU で制御。ゲストの intra-address-space 分離に有用。
- PKS（スーパーバイザーページ用保護キー）: `IA32_PKRS` MSR で制御。Intel Sapphire Rapids 以降サーバーのみ。将来のカーネル内分離に備えて対応を設計する。

**EPT/NPT 実行専用ページ:**

- Intel EPT は execute-only ページ (X=1, R=0, W=0) をサポートし、コード開示を防止できる。AMD NPT は execute-only をサポートしないため、非実行マークとトラップ＆エミュレートによる代替が必要。この差異はコード整合性実装で考慮しなければならない。

**メモリ暗号化への対応方針:**

- Intel TME/TME-MK および AMD SME は物理メモリの AES 暗号化を提供し、物理攻撃に対する保護を強化する。VBS スタイルの保護においてこれらは必須ではないが、対応プラットフォームでは有効化を推奨する。
- AMD SEV-SNP の RMP（Reverse Map Table）と VMPL（Virtual Machine Privilege Level）は、Section 24 の AMD 翻訳整合性経路の補強として利用できる。

### 21.8. IOMMU 統合と DMA 保護

IOMMU なしでは、EPT/NPT によるメモリ分離はすべて DMA 経由でバイパス可能である。これは本設計の最も重大な前提の一つである。

**Intel VT-d:**

- ACPI DMAR テーブルで検出。DMA Remapping（デバイスごとのページテーブル）、Interrupt Remapping（割込み注入攻撃防止）、Posted Interrupts（直接デバイス→vCPU 割込み配信）を提供する。
- Scalable Mode（Skylake-SP 以降）: PASID サポート、first-level/second-level/nested 変換、Scalable IOV を追加。
- マイクロハイパーバイザーは起動時に VT-d を有効化し、すべての DMA 可能デバイスに対して remapping を構成しなければならない。

**AMD-Vi (IOMMU):**

- ACPI IVRS テーブルで検出。v2 機能（Bulldozer 以降）: PASID、PPR、Guest Translation、ATS/IOTLB キャッシュ。
- マイクロハイパーバイザーは AMD-Vi を有効化し、デバイスごとの DMA ドメインを構成しなければならない。

**Kernel DMA Protection:**

- VT-d + UEFI ファームウェア `OPT_IN` フラグ（2019 年以降のシステム）により、外部 DMA 可能デバイス（Thunderbolt、USB4 等）を起動時から分離する。対応プラットフォームでは有効化を必須とする。

**PCIe ACS（Access Control Services）検証:**

- デバイス passthrough 時に、同一 PCIe スイッチ配下のデバイスがピアツーピア DMA を実行し IOMMU をバイパスすることを防止する。ACS が存在しない場合、IOMMU グループの粒度が粗くなり、意図しないデバイスが同一ドメインに含まれる。
- passthrough デバイス割当前に ACS 能力を検証しなければならない。ACS 不備のデバイスは passthrough を許可しない。

**Interrupt Remapping:**

- 割込みリマッピングなしでは、デバイスが任意の CPU に任意のベクタで割込みを注入できる。VT-d/AMD-Vi の Interrupt Remapping を有効化し、すべての MSI/MSI-X 再設定をマイクロハイパーバイザーの制御下に置かなければならない。

### 21.9. 起動整合性: DRTM、Boot Guard/PSB、TPM 2.0

**DRTM（Dynamic Root of Trust for Measurement）:**

- Intel TXT: `GETSEC[SENTER]` により DRTM を確立。Intel 署名 SINIT ACM を CPU キャッシュにロードし、検証済み MLE（Measured Launch Environment）を起動する。vPro プラットフォームで利用可能。CBnT（Converged Boot Guard + TXT）により最新世代にも対応。
- AMD SKINIT: AMD の DRTM 命令。ハードウェアが CPU を測定済みコードパスに強制遷移させる。
- 高保証構成では DRTM を必須とし、マイクロハイパーバイザーのセキュアな起動を保証する。

**Intel Boot Guard / AMD PSB:**

- Intel Boot Guard: OEM 公開鍵ハッシュが CPU e-fuse に焼付け。Initial Boot Block (IBB) をファームウェア実行前に検証。
- AMD PSB（Platform Secure Boot）: OEM 署名鍵を PSP 経由で焼付け。RSASSA-PSS with SHA-384 / 4096-bit 鍵。
- 対応プラットフォームではファームウェア整合性の基盤として Boot Guard/PSB を要求する。

**TPM 2.0:**

- Intel CSME / AMD PSP 経由の fTPM、または discrete TPM。起動測定値の保存、鍵シーリング、リモートアテステーションに使用する。
- 本設計では、ロールバック防止のための NV 領域カウンタ、起動測定チェーン、鍵バインディングに TPM 2.0 を用いる。

### 21.10. 仮想化制御機能の安全な利用

**VMX Preemption Timer（Intel）/ Pause Filter（AMD）:**

- ゲストによる CPU 独占を防止する。スケジューラ公平性に必須。

**NOTIFY VM Exit / Bus Lock VM Exit（Intel Alder Lake 以降）/ Bus Lock Trap（AMD Zen 5）:**

- ゲストがマイクロアーキテクチャストール（機能的 DoS）やバスロック操作（性能 DoS）を引き起こすことを検出する。セキュリティイベントとしてログに記録し、必要に応じてゲストを停止する。

**PML（Page Modification Logging, Intel Broadwell 以降）:**

- ハードウェアによるダーティページ追跡（512 エントリバッファ）。整合性監視に有用。

**Unrestricted Guest（Intel Westmere 以降）:**

- リアルモードゲストサポート。VMM エミュレーション攻撃面を縮小する。

**EPT Violation #VE（Intel Broadwell 以降）:**

- EPT 違反をゲスト例外として配信。ゲスト内セキュリティハンドラに利用可能。

**VMFUNC / EPTP Switching:**

- ゲスト内で EPT ビュー切替を VM exit なしで実行。セキュリティコンパートメント化に有用だが、慎重な制御が必要。

**AMD 固有制御:**

- AVIC / x2AVIC: AMD の APICv 相当。割込み exit を削減。
- vGIF: Virtual Global Interrupt Flag。ネスト仮想化に必要。
- VMCB Clean Bits: VMRUN オーバーヘッド削減。変更追跡の正確性が必要。
- Decode Assists: インターセプト時のハードウェア命令デコード。エミュレーションバグを削減。
- V_NMI (Zen 4): 仮想 NMI マスキング。IRET インターセプトの複雑性を排除。

### 21.11. 先進的セキュリティ機能への対応方針

以下の機能は現行ハードウェアで利用開始されたか、次世代で導入予定である。本設計はこれらを計画対象に含め、将来のインクリメントで統合する。

**Intel LASS（Linear Address Space Separation）:**

- Sierra Forest、Granite Rapids、Arrow Lake、Lunar Lake (2024) で出荷済み。Linux 6.19 でマージ。
- ページング前にクロスモードアドレス空間プローブを防止。KASLR バイパス、TLB タイミング攻撃、SLAM 攻撃を阻止。
- 対応プラットフォームでは有効化を推奨する。

**Intel FRED（Flexible Return and Event Delivery）:**

- Panther Lake (2025 後半/2026 初頭) で初出荷予定。AMD は Zen 6 で採用確認。
- IDT ベースのイベント配信を 4 段イベントスタックと ERETU/ERETS 命令に置換。リング遷移中の一時状態を排除し、NMI 処理を簡素化する。
- 本設計は FRED 対応を将来インクリメントとして計画する。

**Intel PKS（Protection Keys for Supervisor pages）:**

- Intel Sapphire Rapids (2023) 以降サーバーのみ。`IA32_PKRS` MSR。
- カーネル内のドメイン分離に有用。将来の KSI 内部分離に利用検討。

**Intel TDX / AMD SEV-SNP:**

- 機密コンピューティング（VM をハイパーバイザーから保護）。本設計の脅威モデル（OS をハイパーバイザーで保護）とは逆方向だが、AMD SEV-SNP の VMPL は本設計の内部特権階層に利用しうる。TDX Connect / AMD SEV-TIO は将来の trusted I/O に有用。

**Intel LAM / AMD UAI:**

- 線形アドレスのメタデータビット。SLAM 脆弱性のセキュリティリスクが報告されており、LASS と Spectre 緩和の併用なしでは使用してはならない。
...
retained C 実装がこれらの qualification と safe teardown をまだ提供しない段階では、`VM_ASSIGN_DEVICE` と `VM_RELEASE_DEVICE` は `NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない。device qualification matrix と FLR 相当の安全な reset 手順がない build は、passthrough を有効化してはならない。
...
`FBVBS-REQ-0310`  Intel 構成では eIBRS（`IA32_ARCH_CAPABILITIES[1]`）を必須とし、起動時に `IA32_SPEC_CTRL[0]=1` を設定しなければならない。AMD Zen 4 以降では AutoIBRS（`CPUID Fn8000_0021:EAX[8]`）を必須とし、`EFER.AUTOIBRS` を有効化しなければならない。検証方法は boot path test と MSR inspection とする。
`FBVBS-REQ-0311`  異なるパーティション間のコンテキスト切替時に、全物理コアで IBPB（`IA32_PRED_CMD[0]`）を発行しなければならない。検証方法は context switch audit と timing test とする。
`FBVBS-REQ-0312`  Intel 構成で BHI_DIS_S（`IA32_SPEC_CTRL[10]`）対応 CPU では同ビットを有効化しなければならない。BHI_DIS_S 非対応 CPU では VMEXIT および syscall エントリ時にソフトウェア BHB クリアシーケンスを実行しなければならない。検証方法は BHI injection test とする。
`FBVBS-REQ-0313`  Intel eIBRS 搭載 CPU で `IA32_ARCH_CAPABILITIES[24]`（PBRSB_NO）が立っていない場合、VMEXIT 直後に PBRSB 緩和シーケンスを実行しなければならない。検証方法は post-exit prediction test とする。
`FBVBS-REQ-0314`  SMT 有効環境の AMD 構成では STIBP（`IA32_SPEC_CTRL[1]=1`）を明示設定しなければならない。検証方法は cross-thread prediction test とする。
`FBVBS-REQ-0315`  VMEXIT ごとに RSB を 32 エントリで充填しなければならない（Intel）。AMD Zen 1–3 では明示フィル、Zen 4 以降の AutoIBRS は自動クリアで代替可。検証方法は RSB underflow test とする。
`FBVBS-REQ-0316`  L1TF 脆弱な CPU（`IA32_ARCH_CAPABILITIES[0]` = 0）では VM entry 時に L1D フラッシュ（`IA32_FLUSH_CMD[0]`）を実行しなければならない。検証方法は cache side-channel test とする。
`FBVBS-REQ-0317`  MDS/TAA/MMIO Stale Data 脆弱な CPU では、すべての特権遷移時に VERW 命令を実行しなければならない。TAA 脆弱な CPU では TSX 無効化（`IA32_TSX_CTRL`）も併用しなければならない。検証方法は microarchitectural data sampling test とする。
`FBVBS-REQ-0318`  AMD 構成では起動時に `DE_CFG` (MSR 0xC0011029) bit 1 を設定し、LFENCE をディスパッチシリアライズ化しなければならない。検証方法は boot MSR inspection とする。
`FBVBS-REQ-0319`  マイクロハイパーバイザーは起動時に `IA32_ARCH_CAPABILITIES` MSR の全ビットを解析し、per-CPU 脆弱性プロファイルを構築しなければならない。マルチコア環境ではすべての論理プロセッサで一貫性を検証しなければならない。検証方法は multicore MSR consistency test とする。
`FBVBS-REQ-0320`  AMD Zen 1–4 構成では SRSO/Inception 緩和（safe-ret thunk + IBPB）を適用しなければならない。検証方法は speculative return test とする。
`FBVBS-REQ-0321`  Intel Skylake–Tiger Lake 構成では Downfall/GDS 緩和のマイクロコード適用を検証し、`IA32_ARCH_CAPABILITIES[26]`（GDS_NO）の状態を起動時に確認しなければならない。検証方法は boot-time capability check とする。

### G.4.2. 制御フロー完全性要件

`FBVBS-REQ-0330`  CET-SS 対応プラットフォームでは、マイクロハイパーバイザー自身が CET Shadow Stack を使用し、`CR4.CET` をゲストに対してピン留めしなければならない。検証方法は CET disable trap test とする。
`FBVBS-REQ-0331`  CET MSR（`IA32_S_CET`, `IA32_U_CET`, `IA32_PL0_SSP`–`IA32_PL3_SSP`, `IA32_INTERRUPT_SSP_TABLE_ADDR`）は per-vCPU 状態としてパーティション遷移時に保存・復元しなければならない。検証方法は CET state isolation test とする。
`FBVBS-REQ-0332`  シャドウスタックページは EPT/NPT でスーパーバイザーシャドウスタック属性によりマークし、ゲストが直接変更できないようにしなければならない。検証方法は shadow stack EPT protection test とする。
`FBVBS-REQ-0333`  Intel CET-IBT 対応プラットフォームでは、マイクロハイパーバイザー自身のコードに対して IBT を有効化しなければならない。検証方法は indirect branch landing test とする。

### G.4.3. MSR・レジスタ分離要件

`FBVBS-REQ-0340`  Section 21.4 に列挙したセキュリティ重要 MSR（EFER、LSTAR、SPEC_CTRL、CET 関連等）は VMCS/VMCB MSR ビットマップで無条件インターセプトしなければならない。検証方法は MSR access intercept test とする。
`FBVBS-REQ-0341`  VPID/ASID を各 vCPU に一意に割り当て、異なるパーティション間の TLB 情報漏洩を防止しなければならない。検証方法は TLB isolation test とする。
`FBVBS-REQ-0342`  デバッグレジスタ (DR0–DR7) を per-vCPU 状態として厳密に保存・復元し、MOV-DR をインターセプトしなければならない。ゲストのデバッグ状態がホストまたは他パーティションに漏洩してはならない。検証方法は debug register isolation test とする。
`FBVBS-REQ-0343`  `CR4.PCE` を 0 にピン留めし、RDPMC をインターセプトしなければならない。性能カウンタによるサイドチャネルを防止する。検証方法は performance counter intercept test とする。
`FBVBS-REQ-0344`  Intel PT および AMD IBS の構成 MSR をインターセプトし、ゲストが無制限にプロセッサトレースを使用することを禁止しなければならない。検証方法は trace MSR intercept test とする。
`FBVBS-REQ-0345`  `CR4.UMIP` 対応プラットフォームでは同ビットをピン留めし、ゲストが SGDT/SIDT 等を実行して KASLR 情報を取得することを防止しなければならない。検証方法は UMIP enforcement test とする。

### G.4.4. IOMMU・DMA 保護要件

`FBVBS-REQ-0350`  FBVBS v7 適合構成では Intel VT-d または AMD-Vi が有効化され、すべての DMA 可能デバイスに DMA Remapping が設定されなければならない。検証方法は IOMMU configuration test とする。
`FBVBS-REQ-0351`  Interrupt Remapping を有効化し、デバイスによる任意ベクタ/任意 CPU への割込み注入を防止しなければならない。検証方法は interrupt remapping test とする。
`FBVBS-REQ-0352`  passthrough デバイスは、IOMMU グループ、ACS 能力、MSI/MSI-X 制御、reset 能力の検証後にのみ割り当ててよい。ACS 不備のデバイスは passthrough を許可しない。検証方法は device qualification test とする。
`FBVBS-REQ-0353`  Kernel DMA Protection 対応プラットフォームでは、外部 DMA ポート（Thunderbolt、USB4 等）の起動時分離を有効化しなければならない。検証方法は external DMA isolation test とする。

### G.4.5. 起動整合性要件

`FBVBS-REQ-0360`  高保証構成では DRTM（Intel TXT `GETSEC[SENTER]` または AMD SKINIT）によるマイクロハイパーバイザーのセキュア起動を必須とする。検証方法は DRTM boot verification test とする。
`FBVBS-REQ-0361`  Boot Guard（Intel）または PSB（AMD）対応プラットフォームでは、ファームウェア整合性検証が起動チェーンに含まれていることを確認しなければならない。検証方法は firmware integrity inspection とする。
`FBVBS-REQ-0362`  TPM 2.0 が利用可能な構成では、起動測定チェーンの PCR 値をマイクロハイパーバイザー起動時に検証し、期待値と不一致の場合は起動を拒否するかログに記録しなければならない。検証方法は TPM attestation test とする。

### G.4.6. 仮想化制御要件

`FBVBS-REQ-0370`  VMX Preemption Timer（Intel）または Pause Filter（AMD）を構成し、ゲストによる CPU 独占を防止しなければならない。検証方法は guest CPU monopoly test とする。
`FBVBS-REQ-0371`  NOTIFY VM Exit / Bus Lock VM Exit（Intel）または Bus Lock Trap（AMD）対応プラットフォームでは、ゲストのマイクロアーキテクチャ DoS および性能 DoS を検出し、監査ログに記録しなければならない。検証方法は DoS detection test とする。
`FBVBS-REQ-0372`  VM exit/entry 緩和シーケンスは Section 21.3 に定義した順序で実行し、各ステップの省略は per-CPU 脆弱性プロファイルに基づく条件判定によってのみ許可されなければならない。検証方法は exit/entry sequence audit と correctness test とする。
...
本付録は、フルスタック仕様に対する依存関係順序を示す設計付録である。日々の実装進捗、
残課題、リリース判定は `plan/overview/fbvbs-comprehensive-roadmap-2026-03-20.md` を正とし、
スタンドアロン FBVBS マイクロハイパーバイザーの実装順序は
`plan/standalone/implementation/standalone-implementation-plan.md` を正とする。
...
**成果物:**
- C11 + ACSL マイクロハイパーバイザー: パーティション管理、第二レベルページング、IPC、VMCALL ハンドリング、一次監査ログ（UART）
...
- IPC ラウンドトリップ時間の計測
- Frama-C WP と補助的 static analysis によるパーティション管理ロジックの検証
...
ABI v1 で外部可視な列挙値は次のとおり固定する。`SERVICE_KIND_OCS` は共有 ABI 上の
値として予約・使用してよいが、フルスタック適合においてはスタンドアロン管理面拡張
として扱い、本書のサービス平面充足数には数えない。
...
| 名称 | 値/形式 |
|---|---|
| `capability_mask` | `bit 0=partition manage`, `bit 1=memory map`, `bit 2=memory permission set`, `bit 3=shared memory register`, `bit 4=KCI access`, `bit 5=KSI access`, `bit 6=IKS access`, `bit 7=SKS access`, `bit 8=UVS access`, `bit 9=VM manage`, `bit 10=audit/diag`, `bit 11=storage manage`, `bit 12=scale manage`, `bit 13=OCS access` |
...
| `capability_bitmap1` | `bit 0=runtime-ready IOMMU`, `bit 1=measured boot active`, `bit 2=host deprivilege handoff ready`, `bit 3=retained-C foundation ready (VMX + runtime-ready IOMMU + initialized audit path)`, `bit 4=high-assurance foundation ready`。残りは予約であり 0 を返さなければならない |
...
| `event_code` | System events: `1=boot complete`, `2=partition fault`, `3=VM platform gate`, `4=VM device assign`, `5=VM device release`, `6=IOMMU domain create`, `7=IOMMU domain release`, `8=service restart`. Security events (0x80+): `0x80=policy deny`, `0x81=signature reject`, `0x82=rollback detect`, `0x83=DMA deny`, `0x84=VM exit fail-closed`. System and security ranges are globally unique to prevent audit log misclassification. |
...
| `memory_object_id` | microhypervisor memory-object allocator | `MEMORY_MAP`, `MEMORY_REGISTER_SHARED`, `VM_MAP_MEMORY`, `MEMORY_RELEASE_OBJECT` | 解放または再起動まで。ABI v1 では allocating partition に ownership 固定 |
...
`manifest_cbor` は ABI v1 で canonical CBOR map でなければならない。metadata role が `targets` または artifact-carrying role の場合は `component_type`, `target_cpu_vendor`, `required_features`, `target_os_generation`, `payload_hash`, `payload_size`, `generation`, `security_epoch`, `dependencies`, `revocation_reference`, `timestamp`, `snapshot_id`, `role`, `expires_at` を含まなければならない。さらに partition-loadable executable artifact、すなわち `hypervisor`, `trusted-service`, `guest-firmware`, `guest-boot-image` では `entry_ip` を必須とする。`freebsd-module`, `setuid-db`, `policy-fragment` では `entry_ip` は存在してはならない。`freebsd-kernel` は host-executable measurement artifact とし、`entry_ip` を必須としてはならない。`freebsd-kernel` または `freebsd-module` が制限 caller class として hypercall 発行主体になる場合は、追加 key `caller_class`, `load_base`, `allowed_callsites` を必須とする。`root`, `snapshot`, `timestamp`, `revocation` role では、その role に必要な key に加えて `timestamp`, `snapshot_id`, `role`, `expires_at`, `generation`, `security_epoch`, `dependencies`, `revocation_reference` を含まなければならないが、`component_type`, `payload_hash`, `payload_size`, `entry_ip`, `caller_class`, `load_base`, `allowed_callsites` は存在してはならない。省略、不定順整数表現、重複 key は `INVALID_PARAMETER` または `SIGNATURE_INVALID` として扱う。

ABI v1 では、partition-loadable executable artifact を `hypervisor`, `trusted-service`, `guest-firmware`, `guest-boot-image` の四種類に限定する。これらは `entry_ip` を必須とし、後述の固定 loader 規約に従ってロードされなければならない。`freebsd-kernel` は署名検証対象の host-executable measurement artifact であり、`PARTITION_LOAD_IMAGE` を通らない。`freebsd-module` は署名検証対象の code artifact ではあるが bootable executable artifact ではなく、`entry_ip` は存在してはならない。`freebsd-kernel` と `freebsd-module` は host callsite 検証のために `caller_class`, `load_base`, `allowed_callsites` を持たなければならない。`component_type=trusted-service` の manifest には、`service_kind`, `memory_limit_bytes`, `capability_mask`, `vcpu_count`, `initial_sp`, `autostart` を追加必須 key とする。`autostart` は boolean とし、`true` のときのみ boot 時自動生成・自動起動対象となる。`vcpu_count` は 1 以上 252 以下の unsigned integer、`memory_limit_bytes` と `capability_mask` と `initial_sp` は unsigned 64-bit integer とする。

各 key の許可値と符号化規則は次のとおり固定する。artifact-carrying manifest における `component_type` は `hypervisor`, `trusted-service`, `freebsd-kernel`, `freebsd-module`, `guest-firmware`, `guest-boot-image`, `setuid-db`, `policy-fragment` のいずれかの text。`target_cpu_vendor` は `intel`, `amd`, `any` のいずれか。`required_features` は text 配列であり、各要素は `hlat`, `mbec`, `gmet`, `sev-snp`, `iommu`, `cet`, `aesni` のいずれか。`dependencies` は配列であり、各要素は `struct { text component_type; bytes payload_hash; int minimum_generation; }` に対応する CBOR map。`revocation_reference` は UTF-8 text で、失効リスト識別子を表す。`role` は `root`, `targets`, `snapshot`, `timestamp`, `revocation` のいずれか。`snapshot_id` は 32-byte bytes、artifact-carrying manifest の `payload_hash` は SHA-384 の 48-byte bytesとする。partition-loadable executable artifact manifest の `entry_ip` は unsigned 64-bit integer とする。`component_type=trusted-service` のときは追加 key `service_kind` を必須とし、その値は `kci`, `ksi`, `iks`, `sks`, `uvs` のいずれかの text でなければならない。さらに `memory_limit_bytes`, `capability_mask`, `vcpu_count`, `initial_sp`, `autostart` を必須とする。`caller_class` は `fbvbs.ko` または `vmm.ko` の text、`load_base` は unsigned 64-bit integer、`allowed_callsites` は unsigned 64-bit integer 配列で、各要素は 해당 artifact の load base からの callsite offset を表す。trusted time source はマイクロハイパーバイザー起動時に確立された monotonic secure clock とし、`timestamp` と `expires_at` は Unix epoch seconds としてこの clock に対して評価する。ABI v1 では許容 clock skew は 300 秒、信頼時刻が利用不能な場合は `FRESHNESS_FAILED` とする。offline 構成でも secure clock が利用可能でありさえすれば検証してよい。

## L.1.E. Fixed Partition-Loadable Executable Loader Rules
...
| 0x0002 | PARTITION_DESTROY | FreeBSD | `struct { uint64_t partition_id; }` | 空 | `Running` 状態への直接適用は禁止し、fail-closed として `INVALID_STATE` を返す。caller は先に `PARTITION_QUIESCE` を成功させなければならない。`PARTITION_KIND_GUEST_VM` に対しては無効であり `INVALID_PARAMETER` とする。成功時状態は `Destroyed` | `INVALID_PARAMETER`, `NOT_FOUND`, `INVALID_STATE`, `RESOURCE_BUSY`, `TIMEOUT`, `INTERNAL_CORRUPTION` |
...
| 0x0007 | PARTITION_LOAD_IMAGE | FreeBSD | `struct { uint64_t partition_id; uint64_t image_object_id; uint64_t entry_ip; uint64_t initial_sp; }` | 空 | `Measured` から `Loaded`。`image_object_id` は直前に測定済みの object と一致しなければならない。対象は guest VM または trusted-service の partition-loadable executable artifact に限る。`PARTITION_KIND_FREEBSD_HOST` は boot 時 seed され、この call を用いてはならない。partition-loadable executable artifact の配置は Appendix L.1.E の固定 ELF64 loader 規約に従う。`entry_ip=0` の場合は manifest 記録値を用い、`entry_ip!=0` の場合は manifest 記録値および ELF `e_entry` と一致しなければならない。さらに `entry_ip` は executable `PT_LOAD` segment に含まれなければならない。guest VM では `initial_sp` は非ゼロでなければならない。trusted-service では `initial_sp=0` または manifest `initial_sp` との一致のみを許容する。`initial_sp` が指す initial stack page は writable かつ non-executable でなければならず、ABI v1 は automatic stack growth を定義しない。不一致や authoritative image object 不在は fail-closed する | `INVALID_PARAMETER`, `NOT_FOUND`, `INVALID_STATE`, `MEASUREMENT_FAILED`, `RESOURCE_EXHAUSTED`, `RESOURCE_BUSY`, `DEPENDENCY_UNSATISFIED` |
...
| 0x0009 | PARTITION_RECOVER | FreeBSD | `struct { uint64_t partition_id; uint64_t recovery_flags; }` | 空 | `Faulted` から `Runnable`。再測定（失効チェックを含む）、ゼロ化、必要状態復元を伴う。Faulted 中にマニフェストまたはイメージが失効された場合は `REVOKED` を返し回復を拒否する | `INVALID_PARAMETER`, `NOT_FOUND`, `INVALID_STATE`, `MEASUREMENT_FAILED`, `REVOKED`, `RESOURCE_BUSY`, `TIMEOUT`, `INTERNAL_CORRUPTION` |
...
| 0x1001 | MEMORY_MAP | FreeBSD | `struct { uint64_t partition_id; uint64_t memory_object_id; uint64_t guest_physical_address; uint64_t size; uint32_t permissions; uint32_t reserved0; }` | 空 | caller は owner memory object を指定するだけであり、machine frame の最終選択は hypervisor が行う。`shareable` を owner 以外へ map する場合は事前の `MEMORY_REGISTER_SHARED` が必要 | `INVALID_PARAMETER`, `NOT_FOUND`, `INVALID_STATE`, `PERMISSION_DENIED`, `RESOURCE_EXHAUSTED`, `RESOURCE_BUSY` |
| 0x1002 | MEMORY_UNMAP | FreeBSD | `struct { uint64_t partition_id; uint64_t guest_physical_address; uint64_t size; }` | 空 | 指定範囲の map を解除。owner caller または target partition 自身のみ許可 | `INVALID_PARAMETER`, `NOT_FOUND`, `INVALID_STATE`, `PERMISSION_DENIED`, `RESOURCE_BUSY` |
| 0x1003 | MEMORY_SET_PERMISSION | 信頼サービス | `struct { uint64_t target_partition_id; uint64_t guest_physical_address; uint64_t size; uint32_t permissions; uint32_t reserved0; }` | 空 | capability に基づく権限変更。FreeBSD は呼び出してはならない。変更できるのは object owner caller のみであり、peer 権限上限を超えてはならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `PERMISSION_DENIED`, `INVALID_STATE` |
| 0x1004 | MEMORY_REGISTER_SHARED | FreeBSD | `struct { uint64_t memory_object_id; uint64_t size; uint64_t peer_partition_id; uint32_t peer_permissions; uint32_t reserved0; }` | `struct { uint64_t shared_object_id; }` | 双方に明示 capability が必要。registration 自体は権限付与のみであり、peer partition の `mapped_bytes` を消費してはならない。owner caller のみ登録可で、`peer_permissions` は以後の map/permission change の ceiling になる | `INVALID_PARAMETER`, `NOT_FOUND`, `PERMISSION_DENIED`, `RESOURCE_EXHAUSTED`, `ALREADY_EXISTS`, `BUFFER_TOO_SMALL` |
| 0x1005 | MEMORY_RELEASE_OBJECT | FreeBSD | `struct { uint64_t memory_object_id; }` | 空 | owner caller のみ解放可。object がどの partition にも map されず、shared registration も持たないときにのみ解放可 | `INVALID_PARAMETER`, `NOT_FOUND`, `PERMISSION_DENIED`, `INVALID_STATE`, `RESOURCE_BUSY` |
| 0x1006 | MEMORY_UNREGISTER_SHARED | FreeBSD | `struct { uint64_t shared_object_id; }` | 空 | 対応 shared registration を解除する。owner caller のみ解除可。関連 peer map または reserved separate-output 参照が残る間は解放してはならない | `INVALID_PARAMETER`, `NOT_FOUND`, `PERMISSION_DENIED`, `INVALID_STATE`, `RESOURCE_BUSY` |

## L.5. Kernel Code Integrity Service（0x2xxx）

| call_id | 名称 | 呼出元 | 要求本文 | 応答本文 | 前提条件/意味論 | 許可エラー |
|---------|------|--------|----------|----------|-----------------|------------|
| 0x2001 | KCI_VERIFY_MODULE | fbvbs.ko | `struct { uint64_t module_object_id; uint64_t manifest_object_id; uint64_t generation; }` | `struct { uint32_t verdict; uint32_t reserved0; }` | 承認時のみ後続 `KCI_SET_WX` が許可される。`generation` は対象 manifest の `generation` 値と一致しなければならない。ABI v1 では `freebsd-module` manifest に `entry_ip` を持たせてはならず、検証対象は module artifact bytes と manifest metadata の一致のみである。retained C 実装では、host partition 上の当該 module mapping 全体を SHA-384 測定し、artifact catalog の `payload_hash` と一致したときだけ approved module state を更新してよい。実装は raw artifact bytes を authoritative state とし、page-aligned digest table はその内部キャッシュとして保持してよい。失敗時応答本文は 0 長とする。ABI v1 では `status=OK` かつ `verdict=1` のみを承認とし、拒否は必ず non-OK status で表す | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `SIGNATURE_INVALID`, `REVOKED`, `GENERATION_MISMATCH`, `ROLLBACK_DETECTED`, `DEPENDENCY_UNSATISFIED`, `SNAPSHOT_INCONSISTENT`, `FRESHNESS_FAILED`, `BUFFER_TOO_SMALL`, `RESOURCE_EXHAUSTED`, `INVALID_STATE` |
| 0x2002 | KCI_SET_WX | fbvbs.ko | `struct { uint64_t module_object_id; uint64_t guest_physical_address; uint64_t file_offset; uint64_t size; uint32_t permissions; uint32_t reserved0; }` | 空 | 検証済みコードページに限る。`module_object_id` は直前に `KCI_VERIFY_MODULE` で承認済みのものと一致しなければならない。KCI は `file_offset..file_offset+size` の artifact bytes を `guest_physical_address..+size` に配置された bytes と再照合し、一致時のみ execute 権限を付与してよい。ABI v1 では、successful `KCI_VERIFY_MODULE` が構築した raw approved state を authoritative state とし、page-aligned SHA-384 digest table は derived cache としてのみ用いてよい。その場合でも `guest_physical_address == verified_module_base + file_offset` を満たし、対象全 page の digest が derived cache と一致しなければならない。write 付与、unmap、remap は approved state を失効させなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `PERMISSION_DENIED`, `INVALID_STATE`, `NOT_FOUND`, `MEASUREMENT_FAILED` |
| 0x2003 | KCI_PIN_CR | fbvbs.ko | `struct { uint32_t cr_number; uint32_t reserved0; uint64_t pin_mask; }` | 空 | 許可 CR のみ。ABI v1 では caller が追加できるのは must-be-1 pin のみであり、must-be-0 pin（例: `CR4.PCE=0`）は platform security profile の固定方針として hypervisor が保持し、caller が上書きしてはならない。`pin_mask` は追加監視対象 bit を 1 で表す | `INVALID_PARAMETER`, `INVALID_CALLER`, `PERMISSION_DENIED`, `NOT_SUPPORTED_ON_PLATFORM` |
...
| 0x7002 | VM_DESTROY | vmm.ko | `struct { uint64_t vm_partition_id; }` | 空 | guest memory ゼロ化、IOMMU domain 解除。`Created`, `Measured`, `Loaded`, `Runnable`, `Quiesced`, `Faulted` から有効。`Running` への直接適用は禁止し、caller は先に quiesce させなければならない。safe device teardown を満たさない build は assigned device を持つ VM を destroy してはならず、`NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `INVALID_STATE`, `RESOURCE_BUSY`, `TIMEOUT`, `NOT_SUPPORTED_ON_PLATFORM` |
...
| 0x7004 | VM_SET_REGISTER | vmm.ko | `struct { uint64_t vm_partition_id; uint32_t vcpu_id; uint32_t register_id; uint64_t value; }` | 空 | 実行中 vCPU には適用しない。ABI v1 では `VM_REG_RIP`, `VM_REG_RSP`, `VM_REG_RFLAGS`, `VM_REG_CR0`, `VM_REG_CR4` のみ caller 設定可であり、`VM_REG_CR3` は `PERMISSION_DENIED` とする。`VM_REG_RFLAGS` は architectural fixed bit 1 を 1 に保たなければならない。`VM_REG_CR0` と `VM_REG_CR4` は、その時点の active pin policy（must-be-1 / must-be-0 の両方）に一致しなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `INVALID_STATE`, `PERMISSION_DENIED` |
...
| 0x7008 | VM_ASSIGN_DEVICE | vmm.ko | `struct { uint64_t vm_partition_id; uint64_t device_id; }` | 空 | IOMMU group、ACS、interrupt remapping、reset 能力を満たす場合のみ。`device_id` は platform device registry が列挙した opaque ID。`Created`, `Measured`, `Loaded`, `Runnable`, `Quiesced` で有効、`Running` と `Faulted` では無効。qualification matrix や safe reset/FLR を持たない retained C build は成功を返してはならず、`NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `PERMISSION_DENIED`, `POLICY_DENIED`, `NOT_SUPPORTED_ON_PLATFORM`, `RESOURCE_BUSY` |
| 0x7009 | VM_RELEASE_DEVICE | vmm.ko | `struct { uint64_t vm_partition_id; uint64_t device_id; }` | 空 | Function Level Reset または同等手順を実行。`Created`, `Measured`, `Loaded`, `Runnable`, `Quiesced`, `Faulted` で有効、`Running` では無効。safe reset/FLR を実装していない build は成功を返してはならず、`NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `NOT_FOUND`, `INVALID_STATE`, `RESOURCE_BUSY`, `TIMEOUT`, `NOT_SUPPORTED_ON_PLATFORM` |
...
| 0x8004 | DIAG_GET_CAPABILITIES | fbvbs.ko | 空 | `struct { uint64_t capability_bitmap0; uint64_t capability_bitmap1; }` | `capability_bitmap0` は MBEC/GMET、HLAT、CET、AES-NI 等の platform capability、`capability_bitmap1` は runtime-ready IOMMU、measured boot、host deprivilege handoff、initialized audit path を含む retained-C foundation、高保証 foundation の到達状態を返す | `INVALID_CALLER`, `PERMISSION_DENIED`, `BUFFER_TOO_SMALL` |
