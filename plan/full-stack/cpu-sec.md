...
本書は CPU セキュリティ機能に関する調査・比較メモであり、製品要求仕様や適合仕様そのものではない。主に FreeBSD 保護を目的とする FBVBS スタックの設計検討を補助するための参考資料であり、スタンドアロン FBVBS マイクロハイパーバイザーの製品境界、ABI、実装順序、要求適合は `plan/standalone/implementation/standalone-implementation-plan.md`、`plan/standalone/architecture/standalone-runtime-architecture.md`、`plan/standalone/operations/operator-control-plane.md` を正とする。
