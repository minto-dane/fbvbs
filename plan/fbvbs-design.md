...
以下の subsection は、本設計が利用しなければならない x86_64 CPU セキュリティ機能の完全な要求を定義する。これらの機能は `plan/cpu-sec.md` で網羅的に調査された 81 以上の機能に基づく。マルチコア環境において、すべての緩和策はコアごとに適用され、コア間のレース条件を許容してはならない。
