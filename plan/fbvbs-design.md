...
shared registration は capability metadata であり、memory reservation ではない。したがって `MEMORY_REGISTER_SHARED` 成功時点では `memory_limit_bytes` も `mapped_bytes` も消費してはならず、実際に `MEMORY_MAP`、`VM_MAP_MEMORY`、または command separate-output のために map された時点でのみ page-aligned bytes をaccountingしなければならない。shared registration の解除または owner partition の destroy は、まず実際の peer mapping を剥がし、その後に registration metadata を削除することでaccounting整合性を保たなければならない。
...
また、`KCI_SET_WX` は execute 権限を付与する最終境界であるため、artifact bytes と対象 GPA range の実バイト照合ができない実装は、execute 付与を成功させてはならない。byte-backed binding が未実装の段階では、`KCI_SET_WX` は `NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない。
...
| 0x2002 | KCI_SET_WX | fbvbs.ko | `struct { uint64_t module_object_id; uint64_t guest_physical_address; uint64_t file_offset; uint64_t size; uint32_t permissions; uint32_t reserved0; }` | 空 | 検証済みコードページに限る。`module_object_id` は直前に `KCI_VERIFY_MODULE` で承認済みのものと一致しなければならない。KCI は `file_offset..file_offset+size` の artifact bytes を `guest_physical_address..+size` に配置された bytes と照合し、一致時のみ execute 権限を付与してよい。byte-backed 照合を実装していない build は成功を返してはならず、`NOT_SUPPORTED_ON_PLATFORM` で fail-closed しなければならない | `INVALID_PARAMETER`, `INVALID_CALLER`, `PERMISSION_DENIED`, `INVALID_STATE`, `NOT_FOUND`, `MEASUREMENT_FAILED`, `NOT_SUPPORTED_ON_PLATFORM` |
