...
| 7 | Uses syscall restriction if available: OpenBSD `pledge(2)` (OpenBSD only); on FreeBSD use `procctl(PROC_NO_NEW_PRIVS_CTL)` or Capsicum `cap_enter(2)` if available. | **Recommended** | Additional defense-in-depth for syscall surface reduction |
