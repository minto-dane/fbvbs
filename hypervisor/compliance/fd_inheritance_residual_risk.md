...
| 7 | Uses syscall restriction if available: `pledge(2)` (OpenBSD only). On FreeBSD, use `procctl(2)` request `PROC_NO_NEW_PRIVS_CTL` (value `PROC_NO_NEW_PRIVS_ENABLE`) or equivalent process supervision policy. Capsicum `cap_enter(2)` is already covered by requirement #4 and should not be duplicated here. | **Recommended** | Additional defense-in-depth for syscall surface reduction |
