...
| 7 | Uses syscall restriction if available: OpenBSD `pledge(2)` (OpenBSD only); on FreeBSD use the applicable `procctl(2)` restrictions and process supervision policy. Capsicum `cap_enter(2)` is already covered by requirement #4 and should not be duplicated here. | **Recommended** | Additional defense-in-depth for syscall surface reduction without relying on a non-existent `PROC_NO_NEW_PRIVS_CTL` constant |
