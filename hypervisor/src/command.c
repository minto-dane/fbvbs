...
static int fbvbs_validate_command_page(const struct fbvbs_command_page_v1 *page) {
    uint16_t reserved_flags = (uint16_t)(page->flags & (uint16_t)(~FBVBS_CMD_FLAG_SEPARATE_OUTPUT));
    ...
    if (!fbvbs_memory_is_zero(&page->body[page->input_length], sizeof(page->body) - page->input_length)) {
        return INVALID_PARAMETER;
    }
...
static struct fbvbs_command_tracker *fbvbs_get_command_tracker(
    ...
    for (index = 0U; index < FBVBS_MAX_COMMAND_TRACKERS; ++index) {
        if (!state->command_trackers[index].active) {
            fbvbs_zero_memory(&state->command_trackers[index], sizeof(state->command_trackers[index]));
...
static int fbvbs_select_output_buffer(
    ...
    if ((page->flags & FBVBS_CMD_FLAG_SEPARATE_OUTPUT) != 0U) {
        ...
        if (page->output_length_max < required_length) {
        ...
        *buffer = (uint8_t *)(uintptr_t)page->output_page_gpa;
        return OK;
    }
...
static int fbvbs_write_response(
    ...
    status = fbvbs_select_output_buffer(state, owner, page, response_length, &buffer);
    if (status != OK) {
        return status;
    }

    if (response_length > 0U) {
        fbvbs_zero_memory(buffer, response_length);
        fbvbs_copy_memory(buffer, response, response_length);
    }
