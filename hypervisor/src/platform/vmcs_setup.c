...
static volatile uint32_t g_deprivilege_lock;
...
/* ホストの deprivilege 遷移を直列化し、グローバル VMX/EPT 状態が
 * 並行呼び出しで競合しないようにする。 */
static void fbvbs_deprivilege_lock_acquire(volatile uint32_t *lock)
{
#ifdef __FRAMAC__
    if (lock != NULL) {
        *lock = 1U;
...
static void fbvbs_deprivilege_lock_release(volatile uint32_t *lock)
{
#ifdef __FRAMAC__
    if (lock != NULL) {
        *lock = 0U;
    ...
    */
    for (index = 0U; index < FBVBS_MAX_PARTITIONS; ++index) {
        if (state->partitions[index].occupied &&
            ...
            state->partitions[index].state == FBVBS_PARTITION_STATE_RUNNABLE &&
            state->partitions[index].partition_id != 0ULL) {
...
int fbvbs_deprivilege_host(struct fbvbs_hypervisor_state *state)
{
    ...
    fbvbs_deprivilege_lock_acquire(&g_deprivilege_lock);
...
out:
    fbvbs_deprivilege_lock_release(&g_deprivilege_lock);
    return status;
#else
    struct fbvbs_vmcs_config config;
