...
void fbvbs_process_multiboot_info(struct fbvbs_hypervisor_state *state,
    ...
    _Static_assert(sizeof(struct fbvbs_boot_module) == sizeof(state->boot_modules[0]),
                   "fbvbs_boot_module size drift");
    (void)multiboot_info;
    (void)buffer_size;
    state->acpi_rsdp = NULL;
    state->memory_map_count = 0U;
    ...
    state->boot_sub_partition = 0U;
    state->boot_module_count = 0U;
    /*@ loop invariant 0 <= offset <= FBVBS_MAX_BOOT_MODULES;
        loop assigns offset, state->boot_modules[0 .. FBVBS_MAX_BOOT_MODULES - 1];
        loop variant FBVBS_MAX_BOOT_MODULES - offset;
    */
    for (offset = 0U; offset < FBVBS_MAX_BOOT_MODULES; ++offset) {
        state->boot_modules[offset] = (struct fbvbs_boot_module){0};
    }
